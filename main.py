...
def exit_account() -> None:
    log_in = pg.locateCenterOnScreen(path.abspath(path.join(path.dirname(__file__) + '\\img\\' 'swap.png')))
    if log_in:
        pg.moveTo(log_in[0], log_in[1], duration=0.5)
        pg.click()
        pg.sleep(1)
        logout = pg.locateCenterOnScreen(path.abspath(path.join(path.dirname(__file__) + '\\img\\' 'logout.png')))
        pg.moveTo(logout[0], logout[1], duration=0.5)
        pg.click()

def sign_account(login, password) -> None: 
    try:
        input_log = pg.locateCenterOnScreen(path.abspath(path.join(path.dirname(__file__) + '\\img\\' 'input_login.png')))
        if input_log:
            pg.moveTo(input_log[0], input_log[1], duration=0.5)
            pg.click()
            pg.write(login, interval=0.2)
            skip = pg.locateCenterOnScreen(path.abspath(path.join(path.dirname(__file__) + '\\img\\' 'next.png')))
            pg.moveTo(skip[0], skip[1], duration=0.2)
            pg.click()
            pg.sleep(1)
            pg.write(password, interval=0.2)
            pg.sleep(0.3)
            sign_in = pg.locateCenterOnScreen(path.abspath(path.join(path.dirname(__file__) + '\\img\\' 'sign_in.png')))
